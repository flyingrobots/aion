...
set-theoretic and initial-algebra presentation used in our internal
design notes.
...
$k\in\mathbb{N}$, the \emph{$k$-unfolding} of $X$, written $\mathrm{unf}_k(X)$,
replaces each attachment below depth $k$ by an opaque atom while leaving
shallower structure intact.  The \emph{infinite unfolding}
$\mathrm{unf}_\infty(X)$ is the direct limit of the $k$-unfoldings; it is a
possibly infinite tree that still admits the same structural recursion
principles because $\RMG$ is initial for $F$.  These unfoldings let us
reason about recursion schemes (catamorphisms, anamorphisms) over RMGs
without leaving the typed open-graph setting.
