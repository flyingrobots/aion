...
Our technical starting point is algebraic graph transformation via the
double--pushout (DPO) approach in adhesive categories, together with the
Recursive Metagraph (RMG) object model introduced in
...
\begin{enumerate}[leftmargin=*]
  ...
  \item a two-plane concurrent operational semantics with
    ...
    \ref{thm:two-plane}), and---under standard rewrite-theoretic
    assumptions---global confluence (Theorem~\ref{thm:global});
...
motivate the mathematical structure. A companion ``\COMPUTER{}'' paper
will build on these results to define the full machine model and
operating system.
...
\begin{enumerate}[leftmargin=5mm]
  ...
  \item \emph{Two-plane commutation} (Theorem~\ref{thm:two-plane}):
    attachment and skeleton updates commute up to isomorphism via a
    fibration structure;
  \item \emph{Worldline uniqueness} (Corollary~\ref{cor:worldline-uniqueness}):
    uniqueness of complete derivation worldlines up to typed open
    graph isomorphism, extending per-tick commutation to whole-run
    semantics;
