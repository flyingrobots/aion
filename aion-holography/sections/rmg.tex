...
standard graph models and typed open graphs.
...
graph decorated by attachments on vertices and edges.  This
definition agrees with the set-theoretic and initial-algebra
presentation developed in previous technical notes.
...
Let $\mathcal{G}$ be a collection of allowable skeleton shapes
(finite directed multigraphs).  Define a polynomial endofunctor
$F : \Set \To \Set$ by
\[
...
A morphism $f : (S,\alpha,\beta) \To (S',\alpha',\beta')$ consists of:
\begin{itemize}[leftmargin=*]
  ...
  \item for each $v \in V$ a morphism of attachments
    $f_v : \alpha(v) \To \alpha'(f_V(v))$ in $\RMG$, and similarly for
    edges.
\end{itemize}
Composition and identities are defined componentwise.
\end{definition}

This makes $\RMG$ into a category.  In practice, we will not need
arbitrary morphisms; what matters is that monos and pushouts along monos
exist, so that algebraic graph rewriting applies.  We obtain such a
setting by working with typed open graphs and treating an RMG state as a
skeleton in $\OGraph_T$ plus attachment objects in the fibers.
...
\hline\\[-8pt]
$\mathcal{U} = (G;\alpha,\beta)$ & RMG state \\
