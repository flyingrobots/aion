...
We define morphisms by structural recursion on RMG depth.  First form the
discrete category $\mathbf{P}$ with $\mathrm{Ob}(\mathbf{P}) = P$ and
$\mathrm{Mor}(\mathbf{P})$ containing only identity morphisms.  We declare the
RMG hom-sets on atoms by
\[
  ...
  \end{cases}
\]
Under this construction the embedding $\mathrm{Atom} : \mathbf{P} \To \RMG$ is
a faithful functor.
...
\begin{itemize}[leftmargin=*]
  ...
  \item for each $v \in V$ a morphism of attachments
    ...
    $f_e$ is itself an RMG morphism, defined by structural recursion on
    RMG depth (i.e.\ the nesting level in the construction of
    Definition~\ref{def:rmg}).
\end{itemize}
...
\midrule
$\mathcal{U} = (G;\alpha,\beta)$ & single RMG state with $\mathcal{U} \in U$, where an RMG universe $U$ is a collection of RMG states related by rewriting \\
...
Here $U$ always denotes an RMG universe in this sense, with
$\mathcal{U} \in U$ indicating a particular state.
