...
$h \in \Hist(U,R)$ it maps the trace $O_1(h)$ to a trace
...
\[
  \mathrm{dist}_{\mathrm{tr}} : \mathcal{Y} \times \mathcal{Y}
    \to \mathbb{R}_{\ge 0}
\]
be a metric on individual traces.  We lift this pointwise to observers
by defining, for observers $O,O' : \Hist(U,R) \to \mathcal{Y}$,
\[
  \mathrm{Dist}(O,O')
  := \sup_{h \in \Hist(U,R)}
...
\begin{lemma}\label{lem:rulial-basic}
...
\]
In particular, together with \cref{lem:rulial-basic} this makes
