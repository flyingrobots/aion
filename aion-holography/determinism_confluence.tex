...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % baseline graph
    \node[node] (a) at (0,0) {$a$};
    \node[node,del] (b) at (1.8,0.9) {$b$};
    \node[node,use] (c) at (3.6,0) {$c$};
    \node[node] (d) at (1.8,-1.0) {$d$};
    \draw[edge] (a) -- (b);
    \draw[edge] (b) -- (c);
    \draw[edge] (a) -- (d);
    \draw[edge] (d) -- (c);
    ...
    \draw[rounded corners,thick,teal!70]
      ( -0.5, -0.6) rectangle (2.6,1.3);
    \node[anchor=west,teal!70!black] at (2.65,1.25) {$m_1$};

    % match 2
    \draw[rounded corners,thick,orange!80]
      (1.0,-1.4) rectangle (4.2,1.1);
    \node[anchor=west,orange!80!black] at (4.25,1.05) {$m_2$};

    % legend
    \node[node,del] (ld) at (0.4,-2.0) {};
    \node[anchor=west] at (0.7,-2.0) {delete set $\Del(m_i)$};
    \node[node,use] (lu) at (2.6,-2.0) {};
    \node[anchor=west] at (2.9,-2.0) {use set $\Use(m_i)$};
  \end{tikzpicture}
