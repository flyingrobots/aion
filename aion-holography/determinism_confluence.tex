...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % baseline graph
    \node[node] (a) at (0,0) {$a$};
    \node[node,del] (b) at (1.6,0.7) {$b$};
    \node[node,use] (c) at (3.2,0) {$c$};
    \node[node] (d) at (1.6,-0.8) {$d$};
    \draw[edge] (a) -- (b);
    \draw[edge] (b) -- (c);
    \draw[edge] (a) -- (d);
    \draw[edge] (d) -- (c);
    ...
    \draw[rounded corners,thick,teal!70]
      ( -0.4, -0.4) rectangle (2.3,1.1);
    \node[anchor=west,teal!70!black] at (2.35,1.05) {$m_1$};

    % match 2
    \draw[rounded corners,thick,orange!80]
      (1.0,-1.1) rectangle (3.6,0.9);
    \node[anchor=west,orange!80!black] at (3.65,0.85) {$m_2$};

    % legend
    \node[node,del] (ld) at (0.2,-1.6) {};
    \node[anchor=west] at (0.45,-1.6) {delete set $\Del(m_i)$};
    \node[node,use] (lu) at (2.2,-1.6) {};
    \node[anchor=west] at (2.45,-1.6) {use set $\Use(m_i)$};
  \end{tikzpicture}
...
\begin{theorem}[Skeleton-plane tick confluence]\label{thm:tick-confluence}
  ...
  isomorphic successor states.
\end{theorem}

\begin{proof}
By scheduler--admissibility, the skeleton matches
...
Consider a tick on an RMG state $U = (G;\alpha,\beta)$
  ...
  \begin{enumerate}[leftmargin=*]
    ...
    \item[(NC)] (\emph{No clone under descent.})
      ...
      mapped to $x'\in G_S'$, the attachment over $x'$ is obtained from
      the attachment over $x$ solely by the attachment--plane DPOI
...
no-delete/no-clone-under-descent invariant.  Then
